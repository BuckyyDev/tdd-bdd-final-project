...
class TestProductRoutes(TestCase):
    ...
    def test_update_product(self):
        product = self._create_products()[0]
        updated_product_data = product.serialize()
        updated_product_data["name"] = "Updated Fedora"
        updated_product_data["description"] = "An updated red hat"
        updated_product_data["price"] = 15.00
        updated_product_data["available"] = False

        response = self.client.put(f"{BASE_URL}/{product.id}", json=updated_product_data)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_list_products_by_name(self):
        self._create_products(3)
        response = self.client.get(f"{BASE_URL}?name=Fedora")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
